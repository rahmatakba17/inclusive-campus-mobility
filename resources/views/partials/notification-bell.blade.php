...
<div x-data="notificationPanel()" x-init="init()" class="relative" id="notif-wrapper-{{ $variant }}">
    ...
    @if($variant === 'admin')
        <button @click="toggle()" id="notif-btn-admin"
                ...
                :class="open ? 'bg-[#c41e3a] text-white shadow-lg shadow-[#c41e3a]/30' : 'bg-slate-50 text-slate-400 hover:bg-[#c41e3a] hover:text-white hover:shadow-lg hover:shadow-[#c41e3a]/20'"
                title="Notifikasi">
    ...
    @else
        <button @click="toggle()" id="notif-btn-user"
                ...
                :class="open ? 'bg-[#c41e3a] text-white border-[#c41e3a] shadow-lg shadow-[#c41e3a]/30' : 'bg-slate-50 text-slate-400 border-slate-100 hover:bg-[#c41e3a] hover:text-white hover:border-[#c41e3a] hover:shadow-lg hover:shadow-[#c41e3a]/20'"
                title="Notifikasi">
    ...
    <div x-show="open"
        ...
        <div x-show="loading" class="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Memuat...</p>
        </div>
        ...
        <div x-show="!loading && notifications.length === 0"
            ...
            <p class="text-slate-400 text-xs font-medium mt-1 leading-relaxed max-w-[160px]">Tidak ada notifikasi baru untuk ditampilkan.</p>
        </div>

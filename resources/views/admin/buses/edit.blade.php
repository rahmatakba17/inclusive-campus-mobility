...
<div class="max-w-2xl mx-auto">
    <div class="card">
        <div class="flex items-center gap-3 mb-6 pb-6 border-b border-gray-100">
            ...
            <div>
                <h3 class="font-bold text-gray-800">Edit: {{ $bus->name }}</h3>
                <p class="text-sm text-gray-500">Nomor Polisi: {{ $bus->plate_number }}</p>
            </div>
        </div>

        <form action="{{ route('admin.buses.update', $bus) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4" x-data="{ status: '{{ old('status', $bus->status) }}' }">
                ...
                <div>
                    ...
                    <input type="number" name="capacity" id="capacity"
                           class="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-2 text-sm text-gray-500"
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="route" value="{{ old('route', $bus->route) }}" required
                           class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all bg-gray-50 focus:bg-white text-sm">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Jam Berangkat <span class="text-red-500">*</span></label>
                    <input type="time" name="departure_time" value="{{ old('departure_time', $bus->departure_time) }}" required
                           class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all bg-gray-50 focus:bg-white text-sm">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Jam Tiba <span class="text-red-500">*</span></label>
                    <input type="time" name="arrival_time" value="{{ old('arrival_time', $bus->arrival_time) }}" required
                           class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all bg-gray-50 focus:bg-white text-sm">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all bg-gray-50 focus:bg-white text-sm resize-none">{{ old('description', $bus->description) }}</textarea>
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <div x-show="status === 'maintenance'" x-cloak class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl shadow-sm">
                        ...
                        <p class="text-xs text-amber-700 mb-2 font-medium">Laporan ini wajib diisi apabila memindahkan status armada menjadi "Perawatan". Catatan akan diarsipkan di riwayat laporan armada.</p>
                ...
                <div x-data="{ 
                        photoName: null, 
                        photoPreview: '{{ $bus->image ? asset('storage/' . $bus->image) : '' }}',
                        updatePreview(event) {
                    ...
                    <div class="flex items-center gap-6 mt-2">
                        <!-- Preview Box -->
                        <div class="w-32 h-32 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden shrink-0 shadow-inner group relative">
                            <template x-if="!photoPreview">
                                <div class="text-center p-4">
                                    <i class="fas fa-image text-3xl text-gray-300 mb-2 group-hover:text-blue-400 transition-colors"></i>
                        ...
                        <div class="flex-1">
                            ...
                            <button type="button" onclick="document.getElementById('imageInput').click()" 
                                    class="bg-white border border-gray-200 hover:border-blue-500 text-gray-700 font-semibold px-4 py-2.5 rounded-xl text-sm transition-all shadow-sm flex items-center gap-2">
                                <i class="fas fa-upload text-blue-500"></i> Ganti Foto...
                            </button>
            ...
            <div class="flex items-center gap-3 pt-6 border-t border-gray-100">
                <button type="submit"
                        class="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-3 rounded-xl shadow-lg transition-all text-sm">
